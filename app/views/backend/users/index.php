          ...
          <form id="user-edit-form" action="" method="post">
            <div class="modal-body">
              <div class="input-group mb-3">
                ...
                <input id="edit_username" name="username" type="text" class="form-control" placeholder="Username">
              </div>
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-envelope"></i></span>
              ...
              <div class="input-group mb-3">
                ...
                <input id="edit_password" name="password" type="password" class="form-control" placeholder="Leave empty if not change" autocomplete="new-password">
              </div>
              <div class="form-group">
                <label>User Role</label>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#modal-add-user">Add New User</button>
              </div>
              <div class="card-body">
                  ...
                  <tbody>
                    <?php foreach ($users as $key => $value): ?>
                      <tr>
                        ...
                        <td><?= $value->role ?></td>
                        <td>
                          <button class="btn btn-primary btn-sm px-3 mr-2" data-id="<?= $value->id ?>" data-toggle="modal" data-target="#modal-edit-user">Edit</button>
                          <button class="btn btn-danger btn-sm px-2 mr-2" data-id="<?= $value->id ?>" data-toggle="modal" data-target="#modal-delete-user">Delete</button>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
    ...
    $('#modal-edit-user').on('show.bs.modal', function (event) {
      ...
      $.ajax({
        ...
        success: (response) => {
         ...
         $("#edit_role option[value='" + response.role + "']").prop('selected', true);
        },
        error: function(response){
          var errors = response.responseJSON.errors;
          $.each(errors, function(key, value) {
