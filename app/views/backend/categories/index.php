  ...
  <section class="content">
    <div class="card card-primary">
      ...
      <div class=" w-100 d-flex justify-content-between py-2 px-4 bg-primary ">
        ...
        <button type="button" data-toggle="modal" data-target="#modalAddCategory" class="px-3 btn btn-warning text-white font-bold">
        ...
        <table class="table w-100" id="category-table">
          <thead class="w-100 bg-gray border-0">
            <tr>
              ...
              <th scope="col" style="width: 170px!important;">Action</th>
            </tr>
          </thead>
          <tbody class="col-100">
            <?php foreach ($data["categories"] as $key => $value) : ?>
              <tr>
                ...
                <td>
                  <button style="width: 70px;" class="btn btn-sm m-1 btn-warning text-white update" onClick="changeButtonEdit()" id="<?= $value->cate_id ?>">Edit</button>
                  <button style="width: 70px;" class="btn btn-sm m-1 btn-danger  delete" onClick="changeButtonDelete()" id="<?= $value->cate_id ?>">Delete</button>
                </td>
              </tr>
            <?php
            endforeach
...
<section>
  <div class="modal fade" id="modalUpdateCategory" tabindex="-1" role="dialog" aria-labelledby="modalUpdateCategory" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <form id="formModalUpdateCategory" action="" method="POST" enctype="multipart/form-data">
          <div class="modal-body">
            <div class="form-group">
              <label for="name" class="col-form-label">Name <span class="text-red ml-1">*</span></label>
              <input type="text" name="edit_category_name" id="edit_category_name" class="form-control" placeholder="Enter product name">
            </div>
          </div>
          <div class="modal-footer w-100 d-flex justify-between">
    ...
    $.ajax({
      ...
      success: function(data) {
        if (btnChange === "edit") {
          ...
          $("#status_delete").text("Do you want to delete " + data.category_name + "?").addClass( "text-red")
        }
      }
